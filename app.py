...
def index():
    if not session.get("user_id"):
        ...
        joinedChats = c.execute(f"SELECT * FROM rooms, users, user_room WHERE users.user_id=:u_id AND user_room.user_id=:u_id AND rooms.room_id==user_room.room_id",{"u_id": session["user_id"]}).fetchall()
        if request.args.get("search"):
            ...
            return render_template("index.html",joinedChats=joinedChats)
...
def room(room_id):
    if request.method == "POST":
        ...
        private = False
        room = c.execute("SELECT * FROM rooms WHERE room_id=:r_id", {"r_id": room_id}).fetchall()
        chatName = c.execute("SELECT name FROM rooms WHERE room_id=:r_id", {"r_id": room_id}).fetchall()
        joinedChats = c.execute(f"SELECT * FROM rooms, users, user_room WHERE users.user_id=:u_id AND user_room.user_id=:u_id AND rooms.room_id==user_room.room_id",{"u_id": session["user_id"]}).fetchall()
        ...
        return render_template("message.html", messages=messages, private=private, room_id=room_id, users=usernames, joinedChats=joinedChats,chatName=chatName)
